...
class TreeNode(object):
    def __new__(cls, node):
        if not isinstance(node, rt.union_tree):
...
class LeafNode(TreeNode):
    def __new__(cls, arg):
        ...
        result.matrix = None
        if isinstance(arg, rt.union_tree):
...
class NotNode(TreeNode):
    ...
    def build_tree(self):
        node = cta.brlcad_new(rt.struct_tree_node)
        node.magic = rt.RT_TREE_MAGIC
        node.tb_op = rt.OP_NOT
...
class PairNode(TreeNode):
    def __new__(cls, *args):
        if len(args) == 1 and isinstance(args[0], rt.union_tree):
            node = args[0]
...
class SubtractNode(PairNode):
    symbol = "-"
    op_code = rt.OP_SUBTRACT


OP_MAP = {
    rt.OP_DB_LEAF: LeafNode,
    rt.OP_NOT: NotNode,
    rt.OP_UNION: UnionNode,
    rt.OP_INTERSECT: IntersectNode,
    rt.OP_SUBTRACT: SubtractNode,
    rt.OP_XOR: XorNode,
}
